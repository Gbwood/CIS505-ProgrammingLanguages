...
evaluate(add(E1, E2), V) :- evaluate(E1, V1), evaluate(E2, V2), V is V1 + V2.
evaluate(subtract(E1, E2) , V) :- evaluate(E1, V1), evaluate(E2, V2), V is V1 - V2.
evaluate(mult(E1, E2), V) :- evaluate(E1, V1), evaluate(E2, V2), V is V1 * V2.
...
solve(existential(V,C), Sz) :- gen_instances(V, 0, Sz), solve(C, V).
...
gen_instances(V, L, U) :- numlist(L,U,V).
