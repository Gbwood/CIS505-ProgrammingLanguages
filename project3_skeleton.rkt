...
(define (eval exp env)
   (cond
      [(IdentExp? exp)
          (NumVal (lookup-env env exp))]   ;;; CHANGE #1 look up in the environment
      [(NumExp? exp) 
          (NumVal (NumExp-num exp))]
      [(StringExp? exp) 
          (StringVal (StringExp-string exp))]
      [(LambdaExp? exp)
          (ClosureVal (LambdaExp-formal exp) (LambdaExp-body exp) env)]  ;;; CHANGE #2 [lamC (a b) (closV a b env)]
      [(ApplyExp? exp)
          (let ([v1 (eval (ApplyExp-fun exp) env)])
             (cond
                [(ClosureVal? v1)
                   (let ([v2 (eval (ApplyExp-arg exp) env)])
                      (eval (ClosureVal-body v1) (extend-env(ClosureVal-formal v1) v2 (ClosureVal-env v1))))]  ;;; CHANGE #3
                [(NumVal? v1) (display "integer applied as a function\n")]
                [(StringVal? v1) (display "string applied as a function\n")]))]
      [(LetExp? exp)
          (eval (ApplyExp (LambdaExp (LambdaExp-formal exp) (LambdaExp-body exp))
                          (LetExp-exp0 exp))  ;;; CHANGE #4
                 env)]
      [(CondExp? exp)
          ...
          (let ([v1 (eval (PlusExp-exp1 exp) env)]
            ...
            (cond
               ...
               [(and (StringVal? v1) (StringVal? v2))
                   (StringVal (+ (StringVal-str v1) (StringVal-str v2)))] ;;; CHANGE #7
          ...
          (let ([v1 (eval (MinusExp-exp1 exp) env)]
            ...
            (cond
               [(and (NumVal? v1) (NumVal? v2))
                   (NumVal (- (NumVal-num v1) (NumVal-num v2)))]
               [(and (StringVal? v1) (StringVal? v2))
                   (StringVal (- (StringVal-str v1) (StringVal-str v2)))] ;;; CHANGE #8
              ))]
      [(TimesExp? exp)
